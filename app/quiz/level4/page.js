...
export default function Level4() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (scorePercent >= 75) {
      const prevUnlocked = parseInt(localStorage.getItem("unlockedLevel") || "4");
      localStorage.setItem("unlockedLevel", String(Math.max(prevUnlocked, 5)));
    }
  ...
  // Jika JSON kosong, tampilkan error
  if (!questions || questions.length === 0) {
    return (
      <main className="p-10 text-center text-red-600 text-xl">
        ⚠️ ERROR: level4.json kosong / gagal load.
      </main>
    );
  }
                ...
                <div key={q.id} className="p-4 border rounded-lg bg-white shadow">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                    {q.choices.map((c) => {
                      ...
                      const isCorrect =
                        isSelected &&
                        c.toLowerCase() === q.answer.toLowerCase();
